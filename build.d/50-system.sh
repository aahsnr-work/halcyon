#!/usr/bin/env bash
# halcyon build stage 10 — systemd units, services, tmpfiles, chezmoi wiring.
# (ujust machinery + pyprland unit + flatpak user unit are wired in their own
# stages; brew units are retired with brew — MIGRATION §8.7.)
set -euo pipefail
echo "::group::50-system — units + services"
# systemd units shipped in files/systemd/{system,user}
cp -r /ctx/files/systemd/system/. /usr/lib/systemd/system/ 2>/dev/null || true
cp -r /ctx/files/systemd/user/. /usr/lib/systemd/user/ 2>/dev/null || true
...
# chezmoi: first-login init + daily dotfiles update (main's chezmoi module)
