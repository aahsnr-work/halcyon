...
ldd /usr/bin/Hyprland 2>/dev/null | grep -i 'not found' && {
  echo 'ERROR: hyprland -git ABI break'
  exit 1
} || true
...
for p in hyprgraphics hyprlang aquamarine hyprcursor; do
  ...
  echo "$vendor" | grep -qi 'lionheartp' ||
    {
      echo "ERROR: $p vendor '$vendor' — resolved outside the lionheartp COPR; mixed -git stack risk (§1.15). Remediation: raise the COPR repo priority (dnf5 config-manager setopt '*lionheartp*'.priority=1) between copr-enable and install, via two dnf module entries + interleaved script."
      exit 1
    }
done
...
dnf5 repolist --enabled | grep -Ei 'lionheartp' && {
  echo 'ERROR: COPR still enabled'
  exit 1
} || true
