...
v="$(rpm -q --qf '%{VENDOR}' zen-browser)"
echo "$v" | grep -qi 'sneexy' || {
  echo "ERROR: zen-browser vendor '$v' — must be the sneexy COPR (user directive)"
  exit 1
}
v="$(rpm -q --qf '%{VENDOR}' zed)"
echo "$v" | grep -qi 'terra' || {
  echo "ERROR: zed vendor '$v' — must be terra"
  exit 1
}
v="$(rpm -q --qf '%{VENDOR}' code)"
echo "$v" | grep -qi 'microsoft' || {
  echo "ERROR: code vendor '$v' — must be Microsoft"
  exit 1
}
v="$(rpm -q --qf '%{VENDOR}' brave-browser)"
echo "$v" | grep -qi 'brave' || {
  echo "ERROR: brave-browser vendor '$v' — must be Brave"
  exit 1
}
...
ls -d /usr/lib/opt/brave.com 2>/dev/null || ls -d /opt/brave.com 2>/dev/null ||
  {
    echo 'ERROR: brave /opt content missing — check BlueBuild CLI version'
    exit 1
  }
ls /usr/lib/tmpfiles.d/ | grep -i brave || true
# repo hygiene gates (after explicit cleanup above)
dnf5 repolist --enabled | grep -Ei 'brave|^code|zen|terra' && {
  echo 'ERROR: repo cleanup failed'
  exit 1
} || true
ls /etc/yum.repos.d/ | grep -Ei 'brave|vscode|sneexy' && {
  echo 'ERROR: repo file left behind'
  exit 1
} || true
